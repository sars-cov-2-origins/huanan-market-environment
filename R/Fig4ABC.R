...
# Plot
pdf(paste0("../figs/Fig4A.pdf"), width = wpdf.maps + sum(mai.maps[c(2, 4)]) + dLeg, height = hpdf.maps + sum(mai.maps[c(1, 3)]), pointsize = pointsize.maps)

plotBubbleMap(stallNumerators = num, stallDenominators = denom, stallPositions = geom, 
              plotRR = TRUE, rr = risk_RDAmdo_r, 
              logProp = FALSE, titlemap = "Raccoon dog amdovirus")
...
plotBubbleMap(stallNumerators = num, stallDenominators = denom, stallPositions = geom, 
              ...
              logProp = FALSE, titlemap = "Civet kobuvirus", plotLegend = FALSE)
...
plotBubbleMap(stallNumerators = num, stallDenominators = denom, stallPositions = geom, 
              ...
              logProp = FALSE, titlemap = "Bamboo rat coronavirus", plotLegend = FALSE)
