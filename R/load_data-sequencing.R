...
# Read counts
dataMZ <- read.csv("../mtDNA/mitochondrial_metazoa_counts_93.tsv", sep = "\t")
head(dataMZ)
dim(dataMZ)
stopifnot(!any(duplicated(names(dataMZ)))) # Make sure that there are no duplicates

# Covered bases
covMZ <- read.csv("../mtDNA/mitochondrial_metazoa_coveredbases_93.tsv", sep = "\t")
# Information about contig lengths
mt.len <- read.csv("../mtDNA/mitochondrial_lengths.tsv", sep = "\t", header = FALSE)
names(mt.len) <- c("Contig", "Contig_length")

# Load species information
dataSpecies <- read.csv("../mtDNA/species_descriptions_with_common_name.csv", sep = ",")
dim(dataSpecies)
head(dataSpecies)
...
taxo <- read.csv("../mtDNA/speciesTaxonomy.csv")
...
sc2 <- read.csv("../SARS2/sars2_reads_post_trimming.tsv", sep = "\t")
head(sc2)
