...
plotBubbleMap(stallNumerators = num, stallDenominators = denom, stallPositions = geom, sampleSizes = ssize,
              plotRR = FALSE,  
              logProp = TRUE, titlemap = "Human", plotLegend = FALSE)
...
# Plot
pdf(paste0("../figs/Fig3C.pdf"), width = wpdf.maps + sum(mai.maps[c(2, 4)]) + dLeg, height = hpdf.maps + sum(mai.maps[c(1, 3)]), pointsize = pointsize.maps)

plotBubbleMap(stallNumerators = num, stallDenominators = denom, stallPositions = geom, sampleSizes = ssize,
              plotRR = FALSE,  
              logProp = TRUE, titlemap = "Raccoon dog", plotLegend = TRUE)
...
plotBubbleMap(stallNumerators = num, stallDenominators = denom, stallPositions = geom, sampleSizes = ssize,
              plotRR = FALSE,  
              logProp = TRUE, titlemap = "Hoary bamboo rat", plotLegend = FALSE)
...
plotBubbleMap(stallNumerators = num, stallDenominators = denom, stallPositions = geom, sampleSizes = ssize,
              plotRR = FALSE,  
              logProp = TRUE, titlemap = "Masqued palm civet", plotLegend = FALSE)
