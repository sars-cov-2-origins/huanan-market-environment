...
plotBubbleMap <- function(stallNumerators, stallDenominators, stallPositions, plotRR = FALSE, rr = NA, logProp = FALSE, nlg = 11, colorBubbles = baseColorBubbles, titlemap, plotLegend = TRUE, sampleSizes = NA, use_large_points = FALSE){
  ...
  if (!use_large_points) {
    ...
    cexs <- getSampleCex_large(sampleSizes)
  }
  ...
  if(plotLegend){
    ...
    if (!use_large_points) {
      ...
      legend("bottomleft", pt.cex = getSampleCex_large(svals), pch = 21, col = 1, legend = svals, inset = c(-0.03, 0.015), xjust = 0, yjust = 0, 
             cex = 0.9, bty = "n", title = "Number of\nsamples", 
             text.width = txtwd, title.adj = ttladj, pt.lwd=0.5)
    }
