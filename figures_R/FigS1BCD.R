...
plotSpeciesComposition <- function(sID, png = FALSE, pdf = TRUE, format = 'readCount', ntop = 3, refProp = "MZ", keySp = keySpecies){
  ...
  # Plot
  fname <- paste0("../figs/FigS1_", gsub("#", "h", sID), ".png")
  if(format=='prop'){
...
cutoffReadCount <- 400
tmp <- samples[which(samples$Lab.code == 'Q61' | samples$Lab.code == 'Q64'| samples$Lab.code == 'Q68' | samples$Lab.code == 'Q69' | samples$Lab.code == 'Q70' | samples$Lab.code == 'A33' | samples$Lab.code == 'A87' | samples$Lab.code == '8-25-M1' | samples$Lab.code == '8-25-D' | samples$Lab.code == 'W-8-25-D1' | samples$Lab.code == 'W-8-25-L' | samples$Lab.code == 'Q37'), ]
...
speciesOfInterest  <- speciesOfInterest [speciesOfInterest >=cutoffReadCount ]
...
tmp <- samples$Sample.ID[which(is.element(samples$Lab.code, c('Q61', 'Q64', 'Q68', 'Q69', 'Q70', 'A33', 'A87', '8-25-M1', '8-25-D', 'W-8-25-D1', 'W-8-25-L', 'Q37')))] # Sample ID for the lab codes we want
for(sID in tmp){
  if(!is.na(sID)){
    cat(sID, "")
